			...
			<?php if ( 'on' == $display_fromblog_section ) { ?>
				<div id="from-the-blog">
					...
					<?php
						...
						$recent_blog_posts_args = apply_filters( 'et_from_blog_args', array(
							'showposts' => (int) get_option('trim_from_blog_posts_num'),
							'cat' => $recent_fromblog_category_id
						) );
						
						$recent_blog_posts_query = new WP_Query( $recent_blog_posts_args );
						if ( $recent_blog_posts_query->have_posts() ) : while ( $recent_blog_posts_query->have_posts() ) : $recent_blog_posts_query->the_post();
					?>
							<div class="blog-post">
