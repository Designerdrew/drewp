<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
	...
	<article class="entry post clearfix">
		...
		<div class="post-content clearfix">
			<?php
				$thumb = '';
				$width = apply_filters('et_image_width',481);
				$height = apply_filters('et_image_height',230);
				$classtext = '';
	...
	<?php 
		if ( get_option('trim_468_enable') == 'on' ){
			if ( get_option('trim_468_adsense') <> '' ) echo( get_option('trim_468_adsense') );
			   ...
			   <a href="<?php echo esc_url(get_option('trim_468_url')); ?>"><img src="<?php echo esc_url(get_option('trim_468_image')); ?>" alt="468 ad" class="foursixeight" /></a>
	<?php 	}    
		}
	?>
